/**
 * CategoryFormModal - Modal para cadastro de categorias
 *
 * Lógica:
 * - Mantém estados internos para description, purpose, mensagens e validações
 * - Valida campos obrigatórios e tamanho máximo de descrição
 * - Chama a API via createCategory
 * - Exibe mensagens de sucesso ou erro usando ModalGlobal
 *
 * Props:
 * - onClose: função para fechar o modal
 * - onSuccess: função chamada após cadastro bem-sucedido para atualizar lista
 *
 * Estados:
 * - description: texto da categoria
 ...
 * - loading: indica carregamento durante submissão
 * - errors: campos com validação incorreta
 *
 * Layout:
 * - Overlay escuro cobrindo a tela
 * - Modal branco centralizado com bordas arredondadas
 * - Input para descrição e select para finalidade
 * - Botões Cancelar e Salvar
 * - ModalGlobal para exibir mensagens
 */
...
export default function CategoryFormModal({ onClose, onSuccess }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await createCategory({ description, purpose });
